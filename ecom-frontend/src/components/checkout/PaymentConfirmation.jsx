...
const PaymentConfirmation = () => {
    ...
    useEffect(() => {
        ...
        const pendingOrder = JSON.parse(
                localStorage.getItem("PENDING_PAYPAL_ORDER")
        );

        if (!payPalToken || !pendingOrder) {
            return;
        }
    ...
    return (
        <div className='min-h-screen flex items-center justify-center'>
            <div className="p-8 rounded-lg shadow-lg text-center max-w-md mx-auto border border-gray-200">
                ...
                {orderData && (
                    <div className="bg-gray-50 p-4 rounded-md mb-4 text-left">
                        ...
                        <p className="text-sm text-gray-600">
                            ...
                            {orderData.payment?.paymentId || "Pending"}
                        </p>
                ...
                <Link
                    to="/products"
                    className={`inline-block bg-custom-blue text-white font-semibold px-6 py-2 rounded-md hover:opacity-90 transition ${
                        processing
